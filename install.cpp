...
namespace vcpkg::Install
{
    ...
    /// 定义函数以及返回值(print_usage_information)
    /// 包含： bpgh(包构建的结果信息), 定义[printed_usages]为字符串类型, fs(当前文件系统信息), installed(installed路径 (vcpkg/installedpath.h))
    ...
    {
        /// 实现方法
        /// 通过get_cmake_usage()获取fs, installed, bpgh并传给 message 
        auto message = get_cmake_usage(fs, installed, bpgh).message;
        if (!message.empty())
        {
            ///
            /// 如果存在，则将信息传给 existing
    ...
    /// 
    /// 定义 get_cmake_usage(), 参数有 fs, Installed, bpgh
    /// 
    CMakeUsageInfo get_cmake_usage(const Filesystem& fs, const InstalledPaths& installed, const BinaryParagraph& bpgh)
    {
        ...
        /// 
        /// 这里 auto的类型为 vcpkg::path, usage_file() 获取bpgh中的usage文件，如果有并且没有出现错误着打印信息并换行
        ...
        /// 
        /// auto类型为 vector, 使用 filesystem中的read_lines方法来读取bpgh中的installed文件列表并将返回的值传给ec
        auto files = fs.read_lines(installed.listfile_path(bpgh), ec);

        /// 
        /// 如果没有usage文件(及 !ec) 则为header only
        if (!ec)
        {
            ...
            {
                /// 
                /// 获取share下面的所有.cmake结尾的文件
                /// 
                if (Strings::contains(suffix, "/share/") && Strings::ends_with(suffix, ".cmake"))
                {
                    
                    ///
                    ///获取信息， installed绝对路径 = path, find_package_name由三部分组成. 父级目录+filename+sting, contents内容
                    ...
                    {
                        /// 
                        /// 如果没有错误信息, targets的内容为 get_cmake_add_library_names所获取到的信息

                        auto targets = get_cmake_add_library_names(contents);

                        /// 
                        /// 如果 targerts的内容时空的,则target的内容为find_package_name
                    ...
                    /// <summary>
                    /// filename例如： zlib_x86-windows
                    /// 加载config.camke文件 例如： zlib-config.cmake 或者 RapidJSONConfig.cmake
                    /// 
            ...
            if (library_targets.empty())
            {
                ...
                {
                    /// 
                    /// name 经过 sting：：ascii_to_uppercase判断为大写
